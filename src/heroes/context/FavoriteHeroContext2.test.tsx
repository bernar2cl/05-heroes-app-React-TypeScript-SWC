...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <div data-testid="id-favorite">{isFavorite(mockHero).toString()}</div>
    </div>
  );
};
...
describe('FavoriteHeroContext', () => {
  ...
  test('should add hero to favorites when toggleFavorite is called with new Hero', () => {
    ...
    expect(screen.getByTestId('id-favorite').textContent).toBe('true');
  ...
  test('should remove Hero from favorites when toggleFavorite is called', () => {
    ...
    expect(screen.getByTestId('favorite-count').textContent).toBe('0');
    expect(screen.getByTestId('id-favorite').textContent).toBe('false');
